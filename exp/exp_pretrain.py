...
from data_process.pretrain_dataset import Pretrain_Dataset_csv_efficient, Pretrain_Dataset_npy_efficient
import time
...
class UP2ME_exp_pretrain(object):
    ...
    def _get_data(self, flag='train', candidate_patch_num=None):
        ...
        if (not args.efficient_loader): #original implementation used in the paper, generate one dataloader for each length
            loaders = []
            loader_patch_num = []
            ...
            if args.data_format == 'csv':
                ...
                dataset = Pretrain_Dataset_npy_efficient(
                    root_path=args.root_path,
                    data_name=args.data_name,
                    flag=flag,
                    valid_prop=args.valid_prop
            ...
            available_patch_num = []
            for patch_num in candidate_patch_num:
                if dataset.get_start_limit(patch_num*args.patch_size) > 0:
                    available_patch_num.append(patch_num)
            
            print('Use efficient data loader')
            return dataset, available_patch_num
    ...
    def validate(self, val_loaders, val_dataset, val_patch_nums, criterion):
        valid_loss = []
        epoch_time = time.time()
        
        if val_loaders is not None: #original implementation
        ...
        elif val_dataset is not None: #efficient implementation
            for patch_num in val_patch_nums:
                series_loss = []
                select_len = patch_num * self.args.patch_size
                
                #go through all instances with this length
                stop_flag = False; batch_count = 0
                val_dataset.set_iteration_params(select_len, shuffle=True)
                while not stop_flag and (self.args.valid_batches < 0 or batch_count < self.args.valid_batches):
                    batched_ts, batched_channel_idxs, stop_flag = val_dataset.get_next_batch(self.args.batch_size)
                    loss = self.validate_one_batch(batched_ts, batched_channel_idxs, criterion)
                    series_loss.append(loss)
                    batch_count += 1
                valid_loss.append(np.mean(series_loss))
    ...
    def pre_train(self, setting):
        ...
        if (not self.args.efficient_loader): #original implementation
        ...
        else: #efficient implementation
            train_dataset, train_patch_nums = self._get_data('train')
            valid_dataset, val_patch_nums = self._get_data('val')
            train_loaders, valid_loaders = None, None
        ...
        for step in range(self.args.train_steps):
            ...
            if (not self.args.efficient_loader):
                ...
                select_patch_num = random.choice(train_patch_nums) 
                select_len = select_patch_num * self.args.patch_size   
                batch_data = train_dataset.get_random_batch(self.args.batch_size, select_len)
            ...
            if (step + 1) % self.args.valid_freq == 0:
                valid_loss, valid_loss_list, valid_time = self.validate(valid_loaders, valid_dataset, val_patch_nums, criterion)
